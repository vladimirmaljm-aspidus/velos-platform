#!/usr/bin/env bun
/**
 * VELOS — Product Catalog Insertion Script
 *
 * Usage:
 *   bun scripts/velos/insert.ts --dry-run              # validate only, no writes
 *   bun scripts/velos/insert.ts --dry-run --category=metals
 *   bun scripts/velos/insert.ts                         # real insert (all 333)
 *   bun scripts/velos/insert.ts --category=spices       # real insert one category
 *   bun scripts/velos/insert.ts --batch=25 --delay=300  # custom batch size + delay ms
 *
 * Env:
 *   VELOS_API_KEY  (default: the provided key)
 *   VELOS_API_URL  (default: https://velos-platform.vercel.app)
 */
...
const API_BASE = process.env.VELOS_API_URL || "https://velos-platform.vercel.app";
...
async function main() {
  console.log("╔══════════════════════════════════════════════════════════╗");
  console.log("║   VELOS — Product Catalog Seeding                ║");
  console.log("╚══════════════════════════════════════════════════════════╝");
  ...
  if (DRY_RUN) {
    ...
    console.log(`  bun scripts/velos/insert.ts${CATEGORY ? ` --category=${CATEGORY}` : ""}`);
    return;
  }
  ...
  // Write log file (Node fs — works in both Node and Bun runtimes).
  const logFile = `/home/z/my-project/scripts/velos/insert-log-${Date.now()}.json`;
