    ...
    >
      <Page size={pageSize} style={styles.page}>
        {/* ── HEADER (memorandum — fixed, repeats on every page) ────────
            IMPORTANT: the header + footer MUST be inlined directly as
            <View fixed> children of <Page>, NOT wrapped in a component.
            @react-pdf/renderer only recognizes the `fixed` prop on
            direct View children of Page — wrapping it in a function
            component (e.g. <HeaderContent />) causes the renderer to lose
            the fixed signal, so the header/footer only appear on the
            first page and the page-number Text render prop stops working. */}
        {headerEnabled && (
          <View style={styles.header} fixed>
            <View style={styles.headerLeft}>
        ...
        {/* ── FOOTER (memorandum — fixed, repeats on every page) ─────────
            ...
            so @react-pdf/renderer recognizes the fixed prop and repeats
            the footer on every page + renders the page-number Text. */}
        {footerEnabled && (
          <View style={styles.footer} fixed>
            {/* Left column — QR code */}
            <View style={styles.footerColLeft}>
